...
export default function AnalyticsMonitor() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const actions = ["GET /", "GET /api/stats", "DB_READ thoughts", "CACHE_HIT thought:slug"];
  ...
  if (loading) return <div className="p-8 font-mono text-xs text-muted animate-pulse">Initializing monitor...</div>;

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-mono text-xs">
      {/* System Status */}
      <div className="col-span-1 md:col-span-3 bg-[rgba(var(--overlay-invert),0.4)] border border-border p-4 rounded-lg">
        <div className="flex items-center gap-2 text-tertiary mb-4 border-b border-border-subtle pb-2">
          <FaServer />
          <span className="uppercase tracking-widest">System Status</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="flex flex-col gap-1">
            <span className="text-faint uppercase">Total Views</span>
            <span className="text-2xl text-accent-400 font-bold">{stats?.totalViews.toLocaleString()}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-faint uppercase">Thoughts</span>
            <span className="text-2xl text-white font-bold">{stats?.totalThoughts}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-faint uppercase">Published</span>
            <span className="text-2xl text-green-400 font-bold">{stats?.publishedCount}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-faint uppercase">Drafts</span>
      ...
      {/* Top Thoughts */}
      <div className="col-span-1 md:col-span-2 bg-[rgba(var(--overlay-invert),0.4)] border border-border p-4 rounded-lg">
        <div className="flex items-center gap-2 text-tertiary mb-4 border-b border-border-subtle pb-2">
          <FaEye />
          ...
          {stats?.topThoughts.map((thought: any, i: number) => (
            <div key={thought.id} className="flex items-center justify-between p-2 hover:bg-input rounded transition-colors">
              <div className="flex items-center gap-3">
                ...
                <span className="text-secondary truncate max-w-[200px] md:max-w-xs">{thought.title}</span>
              </div>
      ...
      {/* Live Logs */}
      <div className="col-span-1 bg-[rgba(var(--overlay-invert),0.4)] border border-border p-4 rounded-lg flex flex-col">
        <div className="flex items-center gap-2 text-tertiary mb-4 border-b border-border-subtle pb-2">
          <FaTerminal />
        ...
        <div className="flex flex-col gap-1 overflow-hidden relative">
          ...
          {logs.map((log, i) => (
            <div key={i} className={`truncate transition-all duration-500 ${i === 0 ? "text-white" : "text-faint"}`}>
              {log}
            </div>
          ))}
        </div>
