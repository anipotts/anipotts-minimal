...
export const metadata: Metadata = {
  ...
  openGraph: { title: "links | ani potts", description: "All my links in one place", url: "https://links.anipotts.com", siteName: "links.anipotts.com", type: "website" },
};

export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return (
    <html lang="en" className={jetbrainsMono.variable} suppressHydrationWarning>
      <head><ThemeScript /><SubdomainHints current="links" /><SpeculationRules current="links" /></head>
      <body className="relative min-h-screen antialiased text-foreground bg-transparent font-mono selection:bg-accent-400/20 selection:text-accent-400 overflow-x-hidden" suppressHydrationWarning>
        ...
        <PostHogProvider>
          <WindowProvider>
            <WindowLayoutWrapper>
              <WindowContainer>
                <TerminalHeader defaultTitle="ani@potts:~/links.anipotts.com" />
                <WindowInner showNavbar={false} showFooter={false}>
                  <SubdomainHeader subdomain="links" />
                  {children}
                </WindowInner>
                <TerminalStatusBar />
              </WindowContainer>
            ...
            <TerminalNavigator currentSubdomain="links" />
          </WindowProvider>
        </PostHogProvider>
        </ThemeProvider>
      </body>
