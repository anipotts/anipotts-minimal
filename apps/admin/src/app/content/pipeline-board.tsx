...
function ProgressBar({ items }: { items: BoardItem[] }) {
      ...
      <div className="flex h-1.5 rounded-full overflow-hidden bg-zinc-800/60">
        {segments.map((seg) =>
          seg.count > 0 ? (
            <div
              key={seg.status}
              className={`${COLUMN_HEADER_COLORS[seg.status]} transition-all duration-300`}
              style={{ width: `${seg.pct}%` }}
            />
          ) : null,
        )}
...
export default function PipelineBoard({
  ...
  const columnItems = useCallback(
    (status: string) => items.filter((i) => i.status === status),
    [items],
  );
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    // If dropped on a column, overId is the status string
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.id === draggedItem.id ? { ...i, status: targetStatus } : i,
      ),
    );
