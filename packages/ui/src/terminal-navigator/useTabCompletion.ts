...
import { publicSections } from "@anipotts/lib/data";
import { commandNames } from "./commands";

const sectionNames = publicSections.map((s) => s.name);

// Commands that accept a section argument
const sectionArgCommands = new Set(["cd", "open", "ping"]);
...
export function useTabCompletion() {
  const complete = useCallback((input: string): { completed: string; options: string[] } => {
    ...
    // Completing a section argument
    const cmd = parts[0].toLowerCase();
    if (sectionArgCommands.has(cmd)) {
      const partial = parts[parts.length - 1].toLowerCase();
      const matches = sectionNames.filter((s) => s.startsWith(partial));
      if (matches.length === 1) {
